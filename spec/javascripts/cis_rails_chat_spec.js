describe("CisRailsChat", function() {
  var pub, doc;
  beforeEach(function() {
    Faye = {}; // To simulate global Faye object
    doc = {};
    pub = buildCisRailsChat(doc);
  });
  ...
  it("has a fayeExtension which adds matching subscription signature and timestamp to outgoing message", function() {
    ...
    pub.fayeExtension.outgoing(message, function(message) {
      expect(message.ext.cis_rails_chat_signature).toEqual("abcd");
      expect(message.ext.cis_rails_chat_timestamp).toEqual("1234");
      called = true;
    });
