...
require "cis_rails_chat/faye_extension"
require "cis_rails_chat/engine" if defined? Rails
...
module CisRailsChat
  ...
  class << self
    ...
    def publish_message(message)
      raise Error, "No server specified, ensure cis_rails_chat.yml was loaded properly." unless config[:server]
    ...
    def message(channel, data)
      message = {:channel => channel, :data => {:channel => channel}, :ext => {:cis_rails_chat_token => config[:secret_token]}}
    ...
    # Returns a subscription hash to pass to the CisRailsChat.sign call in JavaScript.
